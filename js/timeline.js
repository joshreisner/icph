...
var timeline = {
	...
	init : function(){
		...
		this.slider_end		= this.$slider_eras.last().position().left + this.$slider_eras.last().width();
	...
	move : function() {
		...
		if (newMargin >= timeline.slider_start) {
			newMargin = timeline.slider_start;
			jQuery("#timeline div.arrow.left").hide();
		} else if (jQuery("#timeline div.arrow.left").is(":hidden")) {
			jQuery("#timeline div.arrow.left").show();
		}
		/* if (newMargin <= timeline.slider_end) {
			newMargin = timeline.slider_end;
		...
		window.console.log(timeline.slider_start + "  " + newMargin + "  " + timeline.increment);
	...
	update : function() {
		...
		for (var i = 0; i < this.eras.length; i++) {
			if (this.positions[this.eras[i]] <= currentMargin) {
