jQuery(function(){
	...
	if (jQuery("body").hasClass("timeline")) {
		var $timeline = jQuery('#timeline ul'), timeline_interval, timeline_direction;
		...
		for (var i = 0; i < eras.length; i++) eraLeft[eras[i]] = jQuery("#timeline li#" + eras[i]).position().left;
	...
	function sliderUpdate() {
		...
		var currentMargin = 0 - parseInt($timeline.css("marginLeft")) + sliderLeft;
	...
	function timelineJump(which) {
		...
		$timeline.css("marginLeft", (0 - (eraLeft[which] - sliderLeft)) + "px");
		sliderUpdate();
	}
